                    ...
                    {isEditMode && (
                        <>
                            <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
                                ...
                                <div className="flow-root">{relatedProjects.length > 0 ? (<ul className="-my-4 divide-y divide-border">{relatedProjects.map(project => (<li key={project['Pipeline No']} className="flex items-center space-x-4 py-4"><div className="flex-1 min-w-0"><p className="text-sm font-semibold text-foreground truncate">{project['Pipeline No']}</p><p className="text-sm text-muted-foreground truncate">{project.Requirements}</p></div><div className="text-right"><span className="text-sm font-medium text-foreground">{project['Total Amount']}</span><p className="text-xs text-muted-foreground">{project.Status}</p></div><button type="button" onClick={() => navigateTo('projects', project['Pipeline No'])} className="text-brand-600 hover:text-brand-800 text-sm font-semibold">View</button></li>))}</ul>) : <EmptyState />}</div>
                            </div>
                            <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
