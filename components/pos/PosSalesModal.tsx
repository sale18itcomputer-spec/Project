...
const PosSalesModal: React.FC<PosSalesModalProps> = ({ isOpen, onClose }) => {
  ...
  const [receiptTaxAmount, setReceiptTaxAmount] = useState(0);
  ...
  const receiptByInvNo = useMemo(() => {
    ...
    (receipts ?? []).forEach(r => {
      if (r['Inv No']) map.set(r['Inv No'], r);
    });
    return map;
  }, [receipts]);
  ...
  const openReceipt = (inv: typeof posInvoices[0]) => {
    let items: any[] = [];
    try {
    ...
    const taxType     = (inv['Tax Type'] === 'VAT' ? 'VAT' : 'NON-VAT') as 'VAT' | 'NON-VAT';
    const subTotal    = taxType === 'VAT' ? grandTotal / 1.1 : grandTotal;
    const taxAmount   = taxType === 'VAT' ? grandTotal - subTotal : 0;

    const receipt     = receiptByInvNo.get(inv['Inv No']);
    const rvNo        = receipt?.['RV No'] ?? '';
    const payment     = (receipt?.['Payment Method'] ?? 'Cash') as PosSessionForm['paymentMethod'];
    const currency    = (receipt?.['Currency'] ?? 'USD') as 'USD' | 'KHR';
    ...
    const sale: CompletedSale = {
      ...
      items: items.map((i: any) => ({
        id:           i.id ?? i.itemCode ?? String(Math.random()),
        itemCode:     i.itemCode ?? '',
    ...
    const session: PosSessionForm = {
      ...
      contactName:    inv['Contact Name'] ?? inv['Company Name'] ?? '',
  ...
  return (
    <>
                ...
                <tbody className="divide-y divide-border">
                  {filteredInvoices.length === 0 ? (
                    ...
                    filteredInvoices.map(inv => {
                      ...
                      const payment    = receiptByInvNo.get(inv['Inv No'])?.['Payment Method'] ?? '—';
                      ...
                      return (
                        <React.Fragment key={inv['Inv No']}>
                          ...
                          >
                            ...
                            <td className="px-3 py-2.5 text-center" onClick={e => { e.stopPropagation(); openReceipt(inv); }}>
                              <button
                                className="p-1.5 rounded-lg text-muted-foreground hover:text-brand-500 hover:bg-brand-500/10 transition"
                                title="View Receipt"
                              >
                                <Receipt size={14} />
                              </button>
      ...
      {receiptSession && (
        <PosReceiptModal
          isOpen={!!receiptSale}
          onClose={() => { setReceiptSale(null); setReceiptSession(null); }}
          sale={receiptSale}
