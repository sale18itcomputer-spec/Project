                        ...
                        {column.items.length > 0 ? (
                            column.items.map((item) => (
                                <div
                                    ...
                                    className={`w-full text-left bg-white p-4 rounded-lg shadow-sm hover:shadow-md ${!!onItemMove ? 'cursor-grab active:cursor-grabbing' : 'cursor-pointer'} transition-all duration-200 group relative border-l-4`}
                                    style={{ borderColor: colorConfig[column.color].border.replace('border-', '') }}
                                    role="button"
                                    tabIndex={0}
