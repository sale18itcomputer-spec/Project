...
interface InquiryWindowContentProps {
  windowId: string;
  inquiryId: string | null;
  initialReadOnly: boolean;
}

const InquiryWindowContent: React.FC<InquiryWindowContentProps> = ({ windowId, inquiryId, initialReadOnly }) => {
  const { fetchModule, setProductInquiries, companies, contacts, productInquiries } = useData();
  ...
  const existingInquiry = useMemo(
    () => (inquiryId ? productInquiries?.find(i => i.id === inquiryId) ?? null : null),
    [productInquiries, inquiryId]
  );
  ...
  const loadItems = async (id: string) => {
    const { data } = await supabase.from('inquiry_items').select('*').eq('inquiry_id', id).order('line_number', { ascending: true });
  ...
  useEffect(() => {
    const title = inquiryId
      ? (isReadOnly ? `Inquiry: ${formData.inquiry_no}` : `Editing: ${formData.inquiry_no}`)
      : 'New Product Inquiry';
    ...
    const footer = isReadOnly ? (
      ...
      <div className="flex justify-between items-center w-full">
        ...
        <div className="flex items-center gap-2">
          <button type="submit" form={`inquiry-form-${windowId}`} disabled={isSaving} className="px-4 py-2 rounded-lg border border-border bg-card text-sm font-semibold text-muted-foreground hover:text-foreground transition disabled:opacity-50 flex items-center gap-2">
          ...
          {isAdmin && (formData.status === 'Pending' || formData.status === 'In Progress') && (
            <button type="button" onClick={() => handleSave('Quoted')} disabled={isSaving} className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold transition disabled:opacity-50 flex items-center gap-2 shadow-md">
              Mark as Quoted
            </button>
          )}
        </div>
  ...
  }, [windowId, inquiryId, isReadOnly, isSaving, formData.inquiry_no, formData.status, canSubmit, isAdmin, updateWindow, closeWindow]);
      ...
      <div className="grid grid-cols-1 @lg:grid-cols-2 gap-6">
        ...
        <FormSection title="Inquiry Details">
          ...
          <FormSelect
            ...
            disabled={isReadOnly || !can('product_inquiries', 'edit')}
          />
        </FormSection>
        ...
        <FormSection title="Notes">
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              onChange={(isReadOnly && !isAdmin) ? undefined : handleInputChange}
              rows={4}
          ...
          {items.map((item, index) => (
            <div key={index} className="border border-border rounded-xl overflow-hidden bg-background shadow-sm">
              ...
              <div className="flex items-center gap-3 px-4 py-2.5 bg-muted/30 border-b border-border">
                ...
                {!isReadOnly && (
                  <button type="button" onClick={() => removeItem(index)} className="text-muted-foreground hover:text-rose-500 transition ml-1 flex-shrink-0">
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
...
export default InquiryWindowContent;
